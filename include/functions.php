<?php
	...
	class Functions extends Database {
		...
		function registration($data){
			...
			$query = "insert into registration (name, mobile, email, address, password, created_at) values('".$name."', '".$mobile."', '".$email."', '".$address."', '".$password."', '".$date."')";
		...
		function orderSubmit($data){
			...
			$getsql = "SELECT * FROM cart where user_id = $user_id";
			$getresults = $this->query($getsql);

			while($getdata = $this->fetch($getresults)){
				$product_id = $getdata['product_id'];

				$query = "insert into orders (user_id, product_id, created_at) values ('".$user_id."', '".$product_id."', '".$date."')";
				$this->query($query);
			...
			$userquery = "select * from registration where id=$user_id";
			$userreuslt = $this->query($userquery);
			$userdata = $this->fetch($userreuslt);

			$prodquery = "select * from product where id=$product_id";
			$prodresult = $this->query($prodquery);
			$proddata = $this->fetch($prodresult);

			$mobile = $userdata['mobile'];
			$address = $userdata['address'];
			$email = $userdata['email'];
			$productname = $proddata['name'];
			if(empty($proddata['discount'])){
				$price = $proddata['price'];
			}else{
				$price = $proddata['discount'];
			}
			$recieveremail = 'mukundvishwakarma802@gmail.com';
			...
			// $mail->Body = "Your order for the product ".$productname." of Rs.".$price." has been placed successfully. Your order will be deliverd shortly.";
			// $mail->Send();
			...
			// $mail->IsHTML(true);
			// $mail->Subject = "Order Details";
			// $mail->Body = "You have receieved order of product ".$producname." of Rs.".$price." at address : ".$address;
			// $mail->Send();

			// if(!empty($mobile)){
			// 	$message = "Your order for the product ".$productname." of Rs.".$price." has been placed successfully. Your order will be deliverd shortly.";
			// 	$message = urlencode($message);
			// 	$url = 'http://roundsms.com/api/sendhttp.php?authkey=YTY2OGVkMDI5ZmV&mobiles='.$mobile.'&message='.$message.'&sender=KLSMRD&type=1&route=2';
			// 	// http://roundsms.com/api/sendhttp.php?authkey=YTY2OGVkMDI5ZmV&mobiles=$mobile1&message=$msg&sender=KLSMRD&type=1&route=2
			// 	$smsMsg = $url;
			// 	$sms_delivery = file_get_contents($smsMsg);
				
			// 	$msg = "You have receieved order of product ".$producname." of Rs.".$price." at address : ".$address;
			// 	$msg = urlencode($msg);
			// 	$url1 = 'http://roundsms.com/api/sendhttp.php?authkey=YTY2OGVkMDI5ZmV&mobiles=7400001737&message='.$msg.'&sender=KLSMRD&type=1&route=2';
			// 	$smsMsg1 = $url1;
			// 	$sms_delivery1 = file_get_contents($smsMsg1);
			// }
