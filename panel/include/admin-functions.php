<?php
	...
	class AdminFunctions extends Database {
		...
		function getUniqueProductById($id) {
			$id = $this->escape_string($this->strip_all($id));
			$query = "select * from ".PREFIX."product where id='$id'";
			$sql = $this->query($query);
			return $this->fetch($sql);
		}
		...
		function addProduct($data,$file) {
			...
			$discount = $this->escape_string($this->strip_all($data['discount']));
			...
			$query = "insert into ".PREFIX."product (name ,description, price, discount ,image, created_at) values ('$name', '$description','$price', '$discount', '$image', '$date')";
			return $this->query($query);
		}
		...
		function deleteProduct($id) {
			...
			$this->unlinkImage("product", $Detail['ipad_img'], "large");
			$this->unlinkImage("product", $Detail['phone_img'], "crop");
			$query = "delete from ".PREFIX."product where id='$id'";
			$this->query($query);
			return true;
		}
